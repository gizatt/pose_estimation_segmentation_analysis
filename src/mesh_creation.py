...
def export_sdf(mesh, name, directory, color=[0.75, 0.2, 0.2, 1.],
    ...
    sdf_template = open("src/sdf_template.xml").read()
...
if __name__ == "__main__":
    for k in range(10):
        cyl = create_cut_cylinder(
          radius=0.025/2. + np.random.random()*0.005,
          height=0.037 + np.random.random()*0.005,
          cutting_planes=[([0., 0., 0.], [1., 0., 0.])],
          sections=10)
        export_sdf(cyl, "cyl_%d" % k, "models/half_carrot_models")